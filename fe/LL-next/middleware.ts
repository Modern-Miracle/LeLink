...
export default withAuth(
  async function middleware(req) {
    ...
    console.log('[MIDDLEWARE_DEBUG] User:', token?.user ? {
      id: token.user.id,
    ...
    if (token?.user) {
        ...
        try {
          ...
          if (emailCheckResponse.ok) {
            ...
            if (emailData.success && emailData.data?.resource) {
              ...
              console.log('[MIDDLEWARE_DEBUG] Existing user ID:', emailData.data.userId, 'Current session ID:', user.id);
              ...
              const hasCurrentId = existingIdentifiers.some((id: any) => 
                id.system === 'http://lelink.healthcare/user-id' && id.value === user.id
              );
              ...
              if (!hasCurrentId) {
                ...
                try {
                  const updateResponse = await fetch(
                    `http://localhost:7071/api/fhir-storage/users/${encodeURIComponent(user.email)}/add-identifier`,
                    {
                      method: 'POST',
                      headers: {
      ...
      if (needsOnboarding && pathname !== '/onboarding' && !pathname.startsWith('/api/') && pathname !== '/login' && pathname !== '/' && pathname !== '/privacy' && pathname !== '/terms') {
  ...
  },
  {
    callbacks: {
      authorized: ({ token, req }) => {
        ...
        const publicRoutes = ['/login', '/register', '/', '/privacy', '/terms', '/compliance', '/debug-auth', '/test-auth'];
